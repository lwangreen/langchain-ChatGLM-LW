...
def get_answer(query, vs_path, history, mode, score_threshold=VECTOR_SEARCH_SCORE_THRESHOLD,
    ...
    if mode == "知识库问答" and vs_path is not None and os.path.exists(vs_path) and "index.faiss" in os.listdir(
        ...
        yield [[None, "对话失败，请检查是否配置知识库"]], "" # not return, but yield
...
def init_model():
    ...
    args_dict = vars(args)
    try:
        chat_model.init_cfg(args_dict=args_dict)
...
def reinit_model(llm_model, embedding_model, llm_history_len, no_remote_model, use_ptuning_v2, use_lora, top_k,
    ...
    try:
        ...
        chat_model.init_cfg(history_len=llm_history_len,
                            loader_llm_args={"llm_model": llm_model_ins,
...
def get_vector_store(vs_id, files, sentence_size, history, one_conent, one_content_segmentation):
    ...
    filelist = []
    if chat_model.llm_model_chain and chat_model.embeddings:
        if isinstance(files, list):
            ...
            vs_path, loaded_files = chat_model.init_knowledge_vector_store(filelist, vs_path, sentence_size)
        else:
            vs_path, loaded_files = chat_model.one_knowledge_add(vs_path, files, one_conent, one_content_segmentation,
                                                                   sentence_size)
        if len(loaded_files):
    ...
    return vs_path, None, history + [[None, file_status]], \
           gr.update(choices=chat_model.list_file_from_vector_store(vs_path) if vs_path else [])
    ...
    if vs_id == "新建知识库":
        ...
        if "index.faiss" in os.listdir(vs_path):
            ...
            return gr.update(visible=False), gr.update(visible=False), gr.update(visible=True), \
                   ...
                   gr.update(choices=chat_model.list_file_from_vector_store(vs_path), value=[]), \
...
def reinit_vector_store(vs_id, history):
    try:
        ...
        vs_path, loaded_files = chat_model.init_knowledge_vector_store(os.path.join(KB_ROOT_PATH, vs_id, "content"),
...
def delete_file(vs_id, files_to_delete, chatbot):
    ...
    status = chat_model.delete_file_from_vector_store(vs_path=vs_path,
                                                        filepath=docs_path)
    if "fail" not in status:
        for doc_path in docs_path:
            if os.path.exists(doc_path):
                os.remove(doc_path)
    rested_files = chat_model.list_file_from_vector_store(vs_path)
    if "fail" in status:
        vs_status = "文件删除失败。"
    ...
    chatbot = chatbot + [[None, vs_status]]
    return gr.update(choices=chat_model.list_file_from_vector_store(vs_path), value=[]), chatbot
